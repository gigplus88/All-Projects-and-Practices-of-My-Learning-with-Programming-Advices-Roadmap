...
namespace StudentApiConsoleClient
{
    class Program
    {
        ...
        static async Task Main(string[] args)
        {
            ...
            // Step 1: 
            Console.WriteLine("Enter email (default: ali.ahmed@student.com): ");

            loginRequest.Email = Console.ReadLine() ?? "";    // "ali.ahmed@student.com";

            Console.WriteLine("Enter password (default: password1): ");

            loginRequest.Password = Console.ReadLine() ?? ""; // "password1";



        // Step 2: Call the login endpoint to obtain a JWT token.
        var token = await LoginAndGetTokenAsync(http, loginRequest.Email, loginRequest.Password);
        ...
        static async Task<string> LoginAndGetTokenAsync(HttpClient http, string email, string password)
        {
            ...
            var response = await http.PostAsJsonAsync("/api/Auth/login", request); // Searching the token in the response body of the login endpoint if it exists
            ...
            var tokenResponse = await response.Content.ReadFromJsonAsync<TokenResponse>(); // return obj of type TokenResponse
